...
(s/defn validate-battle-params!
  ...
  (cond (nil? deck1) (throw (ex-info "Deck1 is required"
                                     {:type :validation-error}))
        (nil? deck2) (throw (ex-info "Deck2 is required"
                                     {:type :validation-error}))
        ...
        (or (nil? num-games) (<= num-games 0))
          (throw (ex-info "Number of games must be positive"
                          {:type :validation-error}))
        :else true))

(s/defn simulate-single-game
  :-
  game/GameResult
  [deck1 :- deck/Deck deck2 :- deck/Deck game-id :- s/Str battle-mode]
  (log/info "Starting game simulation:" game-id)
  (try
    (let [initial-state
            (-> (game-state/create-initial-game-state deck1 deck2 game-id)
      ...
      (log/info "Game completed:" game-id
                "Winner:" winner
                "Turns:" (:turn-count final-state))
      {:id game-id,
...
(s/defn simulate-battle-series!
  :-
  battle/BattleResult
  [{:keys [deck1 deck2 num-games battle-mode],
    :or {battle-mode :simple},
    :as params}]
  ...
  (try
    ...
    (let [battle-id (str "battle-" (System/currentTimeMillis))
          ...
          deck1-win-rate
            (if (> num-games 0) (/ (double deck1-wins) num-games) 0.0)
          deck2-win-rate
            (if (> num-games 0) (/ (double deck2-wins) num-games) 0.0)
          avg-game-length (if (seq games)
                            (double (/ (reduce + (map :duration-minutes games))
                                       (count games)))
                            0.0)
          avg-turn-count (if (seq games)
                           (double (/ (reduce + (map :turn-count games))
                                      (count games)))
                           0.0)
...
(s/defn format-battle-summary
  ...
  (let [{:keys [deck1-id deck2-id total-games deck1-wins deck2-wins ties
    ...
    (format
      ...
      deck1-id
      deck1-wins
      (double (* deck1-win-rate 100))
      deck2-id
      deck2-wins
      (double (* deck2-win-rate 100))
      ties
      (double avg-game-length)
      (double avg-turn-count))))