...
(s/defschema House
             (s/enum :brobnar :dis
                     :logos :mars
                     :sanctum :shadows
                     :untamed :star-alliance
                     :saurian :unfathomable
                     :ekwidon :geistoid))

(s/defschema CardType (s/enum :action :artifact :creature :upgrade))

(s/defschema Rarity (s/enum :common :uncommon :rare :special :fixed :variant))
...
(s/defschema KeyforgeApiDeck
             {:data {:id s/Str,
                     :name s/Str,
                     :expansion s/Int,
                     :_links {:houses [s/Str], :cards [s/Str]}},
...
(defn deck->houses [deck] (:houses deck))
...
(defn validate-deck
  ...
  (let [cards (:cards deck)
    ...
    (and (= 36 (count cards))
         (= 3 (count houses))
         (every? #(<= 10 % 14) (vals (frequencies (map :house cards)))))))
