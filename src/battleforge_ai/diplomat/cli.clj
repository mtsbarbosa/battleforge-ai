...
(def cli-options
  [["-h" "--help" "Show help"] ["-v" "--verbose" "Verbose output"]
   ["-c" "--config CONFIG" "Configuration file path" :default "config.edn"]])

(def battle-options
  [["-1" "--deck1 DECK1" "Path to first deck file" :required true]
   ["-2" "--deck2 DECK2" "Path to second deck file" :required true]
   ["-m" "--mode MODE" "Battle simulation mode (simple, in-depth)" :default
    :simple :parse-fn keyword :validate
    [#(contains? (set (battle-modes/list-available-modes)) %)
     (str "Must be one of: "
          (str/join ", " (map name (battle-modes/list-available-modes))))]]
   ["-b" "--battles BATTLES" "Number of battles to simulate" :default 100
    :parse-fn #(Integer/parseInt %)]
   ["-t" "--timeout TIMEOUT" "Timeout per game in minutes" :default 30 :parse-fn
    #(Integer/parseInt %)] ["-o" "--output OUTPUT" "Output file path"]])
...
(def simulate-options
  [["-d" "--decks-dir DIR" "Directory containing deck files" :required true]
   ["-f" "--format FORMAT" "Simulation format (tournament, round-robin, custom)"
    :default "round-robin"]
   ["-b" "--battles-per-match BATTLES" "Battles per matchup" :default 100
    :parse-fn #(Integer/parseInt %)]
   ["-p" "--parallel WORKERS" "Number of parallel workers" :default 4 :parse-fn
    #(Integer/parseInt %)] ["-o" "--output OUTPUT" "Output file path"]])

(def stats-options
  [["-i" "--input INPUT" "Input results file" :required true]
   [nil "--output-format FORMAT" "Output format (json, csv, html)" :default
    "json"]
...
(def fetch-deck-options
  [["-s" "--source SOURCE" "Source to fetch from (keyforge, dok, search)"
    :default "keyforge"]
   ["-o" "--output-dir DIR" "Output directory for deck file" :default "./decks"]
...
(defn usage
  ...
  (->> ["BattleForge AI - Keyforge Deck Battle Simulator" ""
        ...
        "Global options:" options-summary "" "Commands:"
    ...
    (cond
      (:help options)
        (do
          ...
          :help)
      (not deck1-path) (do (println
                             "❌ Error: Deck1 path is required (use --deck1)")
                           :validation-error)
      (not deck2-path) (do (println
                             "❌ Error: Deck2 path is required (use --deck2)")
                           :validation-error)
      :else (let [battle-mode (or (:mode options)
              ...
              (try
                ...
                (let [deck1 (file-storage/load-deck deck1-path)
                  ...
                  (let [battle-result (battle-controller/simulate-battle-series!
                        ...
                        summary (battle-controller/format-battle-summary
                                  battle-result)]
                    (println)
                    (println summary)
                    (when output-path
...
(defn execute-fetch-deck
  ...
  (let [deck-id (first args)
    ...
    (cond (and (not deck-id) (not deck-name))
          ...
          :else (try (println "📡 Fetching deck from" source "...")
                     ;; Call controller instead of handling business logic
                     ;; directly
                     (deck-controller/fetch-deck! {:deck-id deck-id,
                                                   :source source,
...
(defn execute-command
  ...
  (case command
    ...
    (do (println (str "Unknown command: " command)) (System/exit 1))))

(defn -main
  "Main entry point for BattleForge AI CLI"
  [& args]
  (let [{:keys [command command-args options exit-message ok?]} (validate-args
                                                                  args)]
    (if exit-message
