        ...
        (let [deck (cond
                     ...
                     (do
                       (when-not (keyforge-api/validate-deck-uuid deck-id)
                         (throw (ex-info "Invalid Keyforge UUID format" {:uuid deck-id})))
                     ...
                     (do
                       (when-not deck-name
                         (throw (ex-info "Deck name required for search" {})))
                       (dok-api/search-and-fetch-first deck-name))
                     
                     :else
                     (throw (ex-info "Invalid source" {:source source})))
              
              existing? (storage/deck-exists? (:name deck) output-dir)]
          
          (if (and existing? (not overwrite?))
            (println "⚠️  Deck already exists. Use --overwrite to replace it.")
            (let [file-path (storage/save-deck! deck output-dir)]
              (println "✅ Deck saved successfully!")
...
(defn -main
  "Main entry point for BattleForge AI"
  [& args]
