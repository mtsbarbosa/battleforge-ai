...
(s/defn get-current-key-cost
  :-
  s/Int
  "Get the current cost to forge a key (base 6, may be modified later)"
  [_player :- game/Player]
  ;; For now, always 6 amber. In full implementation, this could be
  ;; modified by cards/effects
  key-cost)
...
(s/defn can-forge-key?
  ...
  (let [current-cost (get-current-key-cost player)]
    (and (>= (:amber player) current-cost) (< (:keys player) 3))))
...
(s/defn process-key-phase
  :-
  game/Player
  "Process the key forging phase for a player.
   Key forging is mandatory if the player can afford it."
  [player :- game/Player]
  (if (can-forge-key? player) (forge-key player) player))

(s/defn process-key-forging
  :-
  game/Player
...
(s/defn get-forging-message
  :-
  s/Str
  "Get a message describing the key forging action"
  [player :- game/Player forged? :- s/Bool]
  (if forged?
