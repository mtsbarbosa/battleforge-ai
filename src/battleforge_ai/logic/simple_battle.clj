...
(s/defn play-cards-simple
  :-
  game/Player
  "Play all cards from active house, prioritizing high amber value"
  [player :- game/Player active-house :- deck/House]
...
(defn get-creature-power
  [creature]
  (- (or (:power creature) 0) (or (:damage creature) 0)))

(defn is-destroyed? [creature] (<= (get-creature-power creature) 0))

(defn deal-damage
  [creature damage-amount]
  (update creature :damage (fnil + 0) damage-amount))

(defn remove-destroyed-creatures
  [player]
...
(s/defn use-creatures-simple
  ...
  [game-state :- game/GameState active-house :- deck/House]
  (let [current-player (game-state/get-current-player game-state)
        opponent (game-state/get-opponent game-state)
        my-creatures (filter #(= (:house %) active-house)
                       (:battleline current-player))
        enemy-creatures (:battleline opponent)
        decision
          (strategy/should-fight-or-reap? current-player opponent active-house)
        should-fight?
          (and (= decision :fight) (seq enemy-creatures) (seq my-creatures))]
    (if should-fight?
      (let [fights (map vector my-creatures enemy-creatures)
            [updated-my-creatures updated-enemy-creatures]
              (reduce (fn [[my-acc enemy-acc] [attacker defender]]
                        (let [[damaged-attacker damaged-defender]
                                (fight-creatures attacker defender)]
                          [(conj my-acc damaged-attacker)
                           (conj enemy-acc damaged-defender)]))
                [[] []]
                fights)
            my-non-fighters (drop (count fights) my-creatures)
            enemy-non-fighters (drop (count fights) enemy-creatures)
            updated-my-battleline (vec (concat updated-my-creatures
                                               my-non-fighters
                                               (remove #(= (:house %)
                                                           active-house)
                                                 (:battleline current-player))))
            updated-enemy-battleline (vec (concat updated-enemy-creatures
                                                  enemy-non-fighters))
...
(s/defn execute-play-phase-simple
  :-
  game/GameState
  ...
  (let [current-player (game-state/get-current-player game-state)
        ...
        cards-played (- (count (:hand current-player))
                        (count (:hand player-after-play)))
        state-after-play (game-state/update-current-player game-state
                                                           player-after-play)
        ...
        action-msg (cond (> creatures-fought 0)
                           (format "fights with %d creatures" creatures-fought)
                         (> creatures-reaped 0)
                           (format "reaps with %d creatures" creatures-reaped)
...
(defrecord SimpleBattleHandler []
  battle-modes/BattleModeHandler
    ...
    (play-card [_this player card] (play-card-simple player card))
    (get-mode-name [_this] "Simple Battle")
...
(defn simple-battle-handler [] (->SimpleBattleHandler))