...
(s/defn get-creature-amber-potential
  :-
  s/Num
  [card :- deck/Card]
  (if (= (:card-type card) :creature)
    (or (:expected-amber card)
        (:default-creature-amber (ai-params/get-ai-params)))
    0.0))

(s/defn calculate-house-battleline-potential
  :-
  s/Num
  [battleline :- [deck/Card] house :- deck/House]
  (->> battleline
       (filter #(= (:house %) house))
       (map get-creature-amber-potential)
       (reduce + 0.0)))

(s/defn calculate-battleline-delta
  :-
  s/Num
  ...
  (let [battleline (:battleline player)
        ...
        house-potentials
          (map #(calculate-house-battleline-potential battleline %) houses)
...
(s/defn get-best-battleline-house
  :-
  (s/maybe deck/House)
  [player :- game/Player]
  (let [battleline (:battleline player)
    ...
    (when (seq battleline)
      (->> houses
           (map (fn [house] [house
                             (calculate-house-battleline-potential battleline
                                                                   house)]))
           (sort-by second >)
           (first)
           (first)))))

(s/defn calculate-hand-amber-potential
  :-
  s/Num
  [hand :- [deck/Card] house :- deck/House]
  (->> hand
       (filter #(= (:house %) house))
       (map #(or (:expected-amber %)
                 (:amber %)
                 (:default-creature-amber (ai-params/get-ai-params))))
       (reduce + 0.0)))

(s/defn get-hand-house-potentials
  :-
  {deck/House s/Num}
  [player :- game/Player]
  (let [hand (:hand player)
        houses (:houses player)]
    (into {}
          (map (fn [house] [house (calculate-hand-amber-potential hand house)])
            houses))))

(s/defn can-forge-next-turn?
  :-
  s/Bool
...
(s/defn get-card-amber-control
  :-
  s/Num
  "Heuristic estimate of amber control potential (steal/destroy)"
  [card :- deck/Card]
  (let [params (ai-params/get-ai-params)]
    (cond (and (= (:card-type card) :action) (> (get card :amber 0) 0))
            (:amber-control-action-with-pips params)
          (= (:card-type card) :artifact) (:amber-control-artifact params)
          (and (= (:card-type card) :creature) (> (get card :amber 0) 0))
            (:amber-control-creature-with-pips params)
          :else (:amber-control-default params))))

(s/defn calculate-amber-control-potential
  :-
  s/Num
  [player :- game/Player house :- deck/House]
...
(s/defn get-house-with-most-amber-control
  :-
  deck/House
  [player :- game/Player]
  (let [houses (:houses player)
        house-controls
          (map (fn [house] [house
                            (calculate-amber-control-potential player house)])
            houses)]
    (->> house-controls
         (sort-by second >)
         (first)
...
(s/defn predict-our-amber-potential
  :-
  s/Num
  [player :- game/Player house :- deck/House]
...
(s/defn predict-opponent-amber-potential
  :-
  s/Num
  [opponent :- game/Player]
...
(s/defn should-use-amber-control-strategy?
  :-
  s/Bool
  [current-player :- game/Player opponent :- game/Player]
  (cond (and (can-forge-next-turn? opponent) (is-final-key? opponent)) true
        (can-forge-next-turn? opponent)
          (let [houses (:houses current-player)
                our-best-potential
                  (apply max
                    (map #(predict-our-amber-potential current-player %)
                      houses))
                opponent-potential (predict-opponent-amber-potential opponent)
                multiplier (:opponent-potential-multiplier
                             (ai-params/get-ai-params))]
            (< our-best-potential (* multiplier opponent-potential)))
        :else false))

(s/defn calculate-battleline-power
  :-
  s/Num
...
(s/defn estimate-fight-casualties
  :-
  {:our-losses s/Num, :their-losses s/Num}
  "Estimate amber potential lost by each side in 1-to-1 fights"
  [our-creatures :- [deck/Card] enemy-creatures :- [deck/Card]]
  (let [our-sorted (sort-by #(or (:power %) 0) > our-creatures)
    ...
    (reduce (fn [{:keys [our-losses their-losses]} [attacker defender]]
              (let [our-power (or (:power attacker) 0)
                    their-power (or (:power defender) 0)]
                {:our-losses (if (>= their-power our-power)
                               (+ our-losses
                                  (get-creature-amber-potential attacker))
                               our-losses),
                 :their-losses (if (>= our-power their-power)
                                 (+ their-losses
                                    (get-creature-amber-potential defender))
                                 their-losses)}))
      {:our-losses 0.0, :their-losses 0.0}
      fights)))

(s/defn calculate-delta-swing
  :-
  s/Num
  "Their losses minus our losses. Positive = fight favors us"
  [our-creatures :- [deck/Card] enemy-creatures :- [deck/Card]]
  (let [{:keys [our-losses their-losses]}
          (estimate-fight-casualties our-creatures enemy-creatures)]
    (- their-losses our-losses)))

(s/defn should-fight-or-reap?
  :-
  (s/enum :fight :reap)
  "Fight if delta swing is favorable, reap otherwise"
  [current-player :- game/Player opponent :- game/Player active-house :-
   deck/House]
  (let [params (ai-params/get-ai-params)
        our-creatures (filter #(= (:house %) active-house)
                        (:battleline current-player))
        ...
        we-near-forge? (<= amber-to-forge
                           (:reap-amber-to-forge-threshold params))
        opponent-near-forge? (>= opponent-amber
                                 (:fight-forge-threat-amber params))
        opponent-about-to-win? (and (= (:keys opponent) 2) opponent-near-forge?)
        power-ratio (if (zero? enemy-power)
                      Float/POSITIVE_INFINITY
                      (/ our-power enemy-power))
...
(s/defn should-stick-with-battleline-house?
  :-
  s/Bool
  [current-player :- game/Player opponent :- game/Player]
  (let [current-delta (calculate-battleline-delta current-player)
    ...
    (boolean (and best-battleline-house
                  (> current-delta opponent-delta)
                  (> current-delta 0)
                  (let [high-threshold (:high-threat-threshold
                                         (ai-params/get-ai-params))
                        advantage-req (:battleline-advantage-vs-high-threat
                                        (ai-params/get-ai-params))]
                    (or (<= threat-level high-threshold)
                        (> current-delta (+ opponent-delta advantage-req))))))))

(s/defn should-switch-for-hand-advantage?
  :-
  [(s/one s/Bool "should-switch") (s/one (s/maybe deck/House) "house")]
  [current-player :- game/Player opponent :- game/Player]
  (let [current-delta (calculate-battleline-delta current-player)
        opponent-delta (calculate-battleline-delta opponent)
        threat-level (- opponent-delta current-delta)
        ...
        best-hand-entry
          (reduce (fn [best [house potential]]
        ...
        min-cards-advantage (:min-cards-for-hand-advantage
                              (ai-params/get-ai-params))
        min-cards-switch (:min-cards-for-house-switch
                           (ai-params/get-ai-params))]
    (cond (and (> best-hand-potential
                  (* battleline-potential advantage-threshold))
               (>= (count (filter #(= (:house %) best-hand-house)
                            (:hand current-player)))
                   min-cards-advantage))
          ...
          (and (<= current-delta delta-threshold)
               best-battleline-house
               (= 0
                  (count (filter #(= (:house %) best-battleline-house)
                           (:hand current-player))))
               (>= (count (filter #(= (:house %) best-hand-house)
                            (:hand current-player)))
                   min-cards-switch))
  ...
  (cond
    ...
    :else
      (let [[should-switch? house]
              (should-switch-for-hand-advantage? current-player opponent)]
        (if should-switch?
          house
          (let [current-delta (calculate-battleline-delta current-player)
                opponent-delta (calculate-battleline-delta opponent)
                ...
                high-threshold (:high-threat-threshold
                                 (ai-params/get-ai-params))]
            (if (> threat-level high-threshold)
              (let [hand-potentials (get-hand-house-potentials current-player)
                    best-entry (reduce (fn [best [h p]]
                                         (if (> p (second best)) [h p] best))
                                 [nil 0.0]
                                 hand-potentials)]
                (first best-entry))
              (reduce (fn [best h]
                        (if (> (get house-counts h 0) (get house-counts best 0))
                          h
                          best))
                (first houses)
                houses)))))))