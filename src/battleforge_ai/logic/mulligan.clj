...
(s/defn is-early-game-useless?
  :-
  s/Bool
  "Check if a card is likely useless in early game"
  [card :- deck/Card]
  (or (has-creature-control? card) (has-amber-control? card)))

(s/defn analyze-house-distribution
  :-
  {deck/House s/Int}
...
(s/defn has-bad-house-distribution?
  :-
  s/Bool
...
(s/defn should-mulligan?
  ...
  (let [hand (:hand player)
        hand-size (count hand)
        {:keys [amber-sources playable-cards total-amber useless-early-cards
                house-dist]}
          (reduce (fn [acc card]
                    (let [amber (:amber card)
                      ...
                      (->
                        ...
                        (update :useless-early-cards
                                (if (is-early-game-useless? card) inc identity))
                        (update :house-dist
                                (fn [dist] (update dist house (fnil inc 0)))))))
            {:amber-sources 0,
             :playable-cards 0,
             :total-amber 0,
...
(s/defn process-mulligan-decision
  :-
  game/Player
  "Process AI mulligan decision for a player"
  [player :- game/Player]
  (if (should-mulligan? player) (execute-mulligan player) player))

(s/defn analyze-hand-quality
  :-
  s/Str
  ...
  (let [hand (:hand player)
        hand-size (count hand)
        {:keys [amber-sources playable-cards total-amber creature-count
                action-count useless-early-cards house-dist]}
          (reduce (fn [acc card]
                    (let [amber (:amber card)
                          house (:house card)]
                      (->
        ...
        is-2-2-2 (and (= hand-size 6)
                      ...
                      (every? #(= % 2) (vals house-dist)))]
    (format
      "Hand: %d cards, %s distribution, %d amber sources, %d playable early, %d total amber, %d creatures, %d actions, %d useless early"
      hand-size
