...
(defn load-config-file
  []
  (try (let [config-path "config/ai-tuning.edn"]
         (if (.exists (io/file config-path))
           (edn/read-string (slurp config-path))
           default-ai-params))
       (catch Exception _ default-ai-params)))

(def ^:private cached-ai-params (delay (load-config-file)))

(s/defn get-ai-params :- AIParams [] @cached-ai-params)
...
(s/defn get-advantage-threshold
  :-
  s/Num
  [threat-level :- s/Num]
  (let [params (get-ai-params)]
    (cond (is-high-threat? threat-level) (:high-threat-advantage-threshold
                                           params)
          (is-moderate-threat? threat-level)
            (:moderate-threat-advantage-threshold params)
          :else (:low-threat-advantage-threshold params))))

(s/defn get-delta-threshold
  :-
  s/Num
  [threat-level :- s/Num]
  (let [params (get-ai-params)]
    (cond (is-high-threat? threat-level) (:high-threat-delta-threshold params)
          (is-moderate-threat? threat-level) (:moderate-threat-delta-threshold
                                               params)
