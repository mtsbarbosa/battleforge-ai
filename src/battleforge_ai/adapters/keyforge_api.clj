...
(defn- parse-keywords
  ...
  (if (nil? card-text)
    ...
    (let [valid-keywords #{:elusive :skirmish :taunt :deploy :alpha :omega
                           :hazardous :assault :poison :splash-attack :treachery
                           :versatile}
  ...
  (let [card-id (-> (:card_title api-card)
    ...
    {:id card-id,
     ...
     :amber (or (when-let [aember (:aember api-card)]
                  (if (string? aember) (Integer/parseInt aember) aember))
                0),
     :power (when-let [power (:power api-card)]
              (if (string? power) (Integer/parseInt power) power)),
     :armor (when-let [armor (:armor api-card)]
              (if (string? armor) (Integer/parseInt armor) armor)),
...
(defn extract-uuid-from-url
  ...
  [url]
  (when-let [match (re-find #"decks/([a-fA-F0-9-]{36})" url)] (second match)))
