...
(deftest test-validate-fetch-deck-params!
  ...
  (testing "passes validation for valid params"
    (is (deck-controller/validate-fetch-deck-params! {:source "keyforge",
                                                      :deck-id "test-id"}))
...
(deftest test-handle-deck-storage
  (testing "returns already-exists status when deck exists and no overwrite"
    (with-redefs [battleforge-ai.diplomat.file-storage/deck-exists? (constantly
                                                                      true)]
      (let [result (deck-controller/handle-deck-storage {:name "test-deck"}
                                                        {:output-dir "test",
                                                         :overwrite? false})]
  ...
  (testing "saves deck when overwrite is true"
    (with-redefs [battleforge-ai.diplomat.file-storage/deck-exists? (constantly
                                                                      true)
                  battleforge-ai.diplomat.file-storage/save-deck!
                    (constantly "test/path")]
      (let [result (deck-controller/handle-deck-storage {:name "test-deck"}
                                                        {:output-dir "test",
                                                         :overwrite? true})]
