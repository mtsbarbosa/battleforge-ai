...
(deftest test-validate-args
  ...
  (testing "valid command with args"
    (let [result (cli/validate-args ["battle" "--deck1" "deck1.json" "--deck2"
                                     "deck2.json"])]
...
(deftest test-cli-options
  (testing "battle options parsing" (is (some? cli/battle-options)))
  (testing "simulate options parsing" (is (some? cli/simulate-options)))
  (testing "stats options parsing" (is (some? cli/stats-options)))
  (testing "fetch-deck options parsing" (is (some? cli/fetch-deck-options))))
