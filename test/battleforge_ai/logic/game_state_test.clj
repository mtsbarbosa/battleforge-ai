...
(deftest test-create-initial-game-state
  (testing "create-initial-game-state creates valid game state"
    (let [game-state (game-state/create-initial-game-state sample-deck-1
                                                           sample-deck-2
                                                           "test-game")]
...
(deftest test-switch-active-player
  (testing "switch-active-player toggles between players"
    (let [initial-state (game-state/create-initial-game-state sample-deck-1
                                                              sample-deck-2
                                                              "test-game")
...
(deftest test-update-player
  (testing "update-player correctly updates the specified player"
    (let [game-state (game-state/create-initial-game-state sample-deck-1
          ...
          updated-state
            (game-state/update-player game-state :player1 updated-player)]
      ...
      (let [updated-player2 (assoc (get-in game-state [:player2]) :keys 2)
            updated-state2
              (game-state/update-player game-state :player2 updated-player2)]
...
(deftest test-add-to-game-log
  (testing "add-to-game-log appends messages to game log"
    (let [game-state (game-state/create-initial-game-state sample-deck-1
      ...
      ;; Add another message
      (let [double-logged (game-state/add-to-game-log logged-state
                                                      "Second message")]
...
(deftest test-game-state-immutability
  (testing "game state functions maintain immutability"
    (let [original-state (game-state/create-initial-game-state sample-deck-1
                                                               sample-deck-2
                                                               "test-game")
          ...
          player-updated-state
            (game-state/update-player original-state :player1 updated-player)]
...
(deftest test-shuffle-deck-with-hand
  (testing
    "shuffle-deck-with-hand combines hand and deck, shuffles, and draws specified cards"
    (let [player {:id "player1",
                  :deck [sample-card-1 sample-card-2],
                  :hand [sample-card-3],
                  :discard [],
  ...
  (testing
    "shuffle-deck-with-hand draws all cards when requested more than available"
    (let [player {:id "player1",
                  :deck [sample-card-1],
                  :hand [sample-card-2],
                  :discard [],
...
(deftest test-shuffle-deck-with-hand-randomness
  (testing "shuffle-deck-with-hand produces different results (randomness test)"
    (let [player {:id "player1",
      ...
      ;; Note: This test may occasionally fail due to randomness, but it's
      ;; very unlikely
