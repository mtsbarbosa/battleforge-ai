...
(deftest test-should-mulligan-low-quality-hand
  (testing "should-mulligan? returns true for low quality hand"
    (let [bad-hand [no-amber-card no-amber-card no-amber-card
                    no-amber-card no-amber-card no-amber-card
                    no-amber-card]
          player (create-test-player bad-hand [high-amber-card high-amber-card])]
...
(deftest test-should-mulligan-edge-cases
  (testing "should-mulligan? edge cases"
    ...
    (let [player (create-test-player [] [high-amber-card medium-amber-card low-amber-card])]
      (is (true? (mulligan/should-mulligan? player))))
...
(deftest test-execute-mulligan
  (testing "execute-mulligan shuffles hand back and draws (hand-size - 1) cards"
    (let [initial-hand [high-amber-card medium-amber-card low-amber-card
      ...
      ;; New deck should have 3 cards (original 2 + 7 shuffled back - 6 drawn)
      (is (= 3 (count (:deck result))))
      
      ;; Total cards should be preserved
      (is (= 9 (+ (count (:hand result)) (count (:deck result)))))
      
      ;; Hand cards should be different from original (with very high probability)
...
(deftest test-has-bad-house-distribution
  (testing "has-bad-house-distribution? detects 2/2/2 pattern"
    ...
    (let [good-hand [high-amber-card medium-amber-card low-amber-card  ; 3 different houses
...
(deftest test-creature-control-detection
  (testing "has-creature-control? detects high creature control via SAS metrics"
    (is (true? (mulligan/has-creature-control? board-wipe-card)))  ; creature-control = 4
    (is (false? (mulligan/has-creature-control? high-amber-card)))  ; creature-control = 0
...
(deftest test-amber-control-detection
  (testing "has-amber-control? detects high amber control via SAS metrics"
    (is (true? (mulligan/has-amber-control? amber-control-card)))  ; amber-control = 3
    (is (false? (mulligan/has-amber-control? board-wipe-card)))    ; amber-control = 0
...
(deftest test-early-game-useless-cards
  (testing "is-early-game-useless? identifies problematic early cards"
    (is (true? (mulligan/is-early-game-useless? board-wipe-card)))    ; has creature-control > 2
    (is (true? (mulligan/is-early-game-useless? amber-control-card))) ; has amber-control > 1
    (is (false? (mulligan/is-early-game-useless? high-amber-card)))   ; both controls = 0
    (is (false? (mulligan/is-early-game-useless? medium-amber-card))) ; both controls = 0
    ...
    (let [edge-creature-control {:id "edge-creature"
                                 ...
                                 :image nil
                                 :creature-control 2
                                 :amber-control 0}  ; exactly 2, should be false
          edge-amber-control {:id "edge-amber"
                              ...
                              :image nil
                              :creature-control 0
                              :amber-control 1}]       ; exactly 1, should be false
...
(deftest test-should-mulligan-too-many-useless-cards
  (testing "should-mulligan? flags hands with too many early-game useless cards"
    (let [too-many-useless [board-wipe-card amber-control-card board-wipe-card  ; 3 useless
                            high-amber-card medium-amber-card low-amber-card]   ; 3 good
